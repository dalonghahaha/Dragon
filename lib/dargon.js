...
class dargon {
    ...
    get_default_path(request) {
        var view_path = path.resolve('./views/' + this._request.controller + "/" + this._request.action + '.' + application['tpl_ext']);
        return view_path;
    }

    render(data, tpl_path) {
        var view_path = tpl_path ? path.resolve('./views/' + tpl_path + '.' + application['tpl_ext']) : this.get_default_path();
