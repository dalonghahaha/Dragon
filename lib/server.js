...
class server {
    ...
    init() {
        ...
        this.instance = http.createServer(function(request, response) {
            response.setTimeout(60000, function() {
                this.error_handler(504, response);
            });
    ...
    worker(request, response) {
        //路由
        var route = require(path.join(__dirname,"route",this.route));
        ...
        //执行
        this.get_dragon(destiny,request).then(function(_dragon){
